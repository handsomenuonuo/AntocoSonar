 ...
 * @Describe:
 **********************************/
internal class PipeRenderer(val context: Context): Renderer {

    private var needInitVao = false
...
//        Matrix.orthoM(projectionM, 0, -aspectRatio, aspectRatio,-1f, 1f,  -1f, 10f)
        //设置透视投影
        Matrix.frustumM(projectionM, 0, -aspectRatio, aspectRatio, -1f, 1f, 1f, 1000f)

    }

    private var length = 0f //当前绘制的管道的总长度

    private var localEyeLoc = 30f
    private var eyeLoc = localEyeLoc
    fun scale(scale :Float){
        val d = localEyeLoc / scale
        eyeLoc = if(d < (length+1)/2f) (length+1)/2f else d
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        synchronized(obj){
            vertexBuffer?.let {
                ...
                //设置相机位置
                Matrix.setLookAtM(viewM, 0, tranX, tranY, eyeLoc, tranX, tranY, length/2f, 0f, 1f, 0f);
