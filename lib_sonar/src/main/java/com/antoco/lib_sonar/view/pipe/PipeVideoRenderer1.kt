 ...
 * @Describe:
 **********************************/
internal class PipeVideoRenderer1(val context: Context): Renderer {
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        mProgram = MGl30Utils.createProgram(context, "PipeVShader1.glsl", "PipeFShader1.glsl")

        GLES30.glUseProgram(mProgram)
        textureHandle = GLES30.glGetUniformLocation(mProgram, "s_TextureMap")
        loadTexture()
    }
...
//将纹理单元和纹理对象进行绑定
    ...
    override fun onDrawFrame(gl: GL10?) {
        synchronized(obj){
            vertexBuffer?.let {
                ...
                val u_MVPMatrix = GLES30.glGetUniformLocation(mProgram, "u_MVPMatrix")
                val u_ModelMatrix = GLES30.glGetUniformLocation(mProgram, "u_ModelMatrix")
                // 输入变换矩阵信息
                GLES30.glUniformMatrix4fv(u_MVPMatrix, 1, false, resultM1, 0)
                GLES30.glUniformMatrix4fv(u_ModelMatrix, 1, false, modelM, 0)

                /**===========以下代码用于设置光照效果=============*/
                // 设置光源的位置、颜色和方向
                GLES30.glUniform3f(GLES30.glGetUniformLocation(mProgram, "light.position"), 0f, 2f, 3f)
                GLES30.glUniform3f(GLES30.glGetUniformLocation(mProgram, "light.color"), 1.0f, 1.0f, 1.0f)
                GLES30.glUniform3f(GLES30.glGetUniformLocation(mProgram, "light.direction"), 0.0f, 0f, -1.0f)

                // 用于计算边缘的过度，cutOff 表示内切光角，outerCutOff 表示外切光角
                GLES30.glUniform1f(GLES30.glGetUniformLocation(mProgram, "light.cutOff"), kotlin.math.cos(
                    Math.toRadians(70.0)).toFloat()
                )
                GLES30.glUniform1f(GLES30.glGetUniformLocation(mProgram, "light.outerCutOff"),
                    kotlin.math.cos(
                        Math.toRadians(71.0)).toFloat()
                )
                // 衰减系数,常数项 constant，一次项 linear 和二次项 quadratic。
                GLES30.glUniform1f(GLES30.glGetUniformLocation(mProgram, "light.constant"),  0.1f)
                GLES30.glUniform1f(GLES30.glGetUniformLocation(mProgram, "light.linear"),    0.06f)
                GLES30.glUniform1f(GLES30.glGetUniformLocation(mProgram, "light.quadratic"), 0.01f)
                ...
                GLES30.glDrawElements(GLES30.GL_TRIANGLES, indicesBuffer!!.limit(), GLES30.GL_UNSIGNED_INT, indicesBuffer)
    ...
    fun setData(vertex: FloatArray, indices: IntArray,normals : FloatArray) {
        synchronized(obj){
            ...
            normalBuffer?.clear()
            if(normalBuffer == null || normalBuffer!!.limit() != normals.size){
